...
fprintf(fid, '| Surface-3 prototype | 9 data qubits | X-error syndrome model | Minimum-weight lookup corrects single X errors |\n\n');
...
fprintf(fid, '\n## Surface-3 Prototype\n\n');
fprintf(fid, 'Small X-error-only surface-code model with cached minimum-weight correction lookup.\n\n');
ps_surface = [0.02 0.08 0.14];
Nsurf = 500;
[~, fail_surface] = sweep_surface3_x_logical_error(ps_surface, Nsurf);
fprintf(fid, '| X error probability | Trials | Logical failure |\n');
fprintf(fid, '| ---: | ---: | ---: |\n');
for i = 1:numel(ps_surface)
    fprintf(fid, '| %.2f | %d | %.3f |\n', ps_surface(i), Nsurf, fail_surface(i));
end
